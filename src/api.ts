...
import { RetryNb, RetryTempo, RetryTempoFirst, TimeOut, UrlAuth, UrlConsultation, UrlDepot } from "@/utils/conts";
...
export class ApiDpae {
    ...
    public async send(): Promise<boolean> {
        ...
        if (this.Employee.BirthDepartment.length > 2) {
            this.Employee.BirthDepartment = this.Employee.BirthDepartment.substring(0, 2);
        }
    ...
    public async retour(retry: number = 0): Promise<boolean> {
        ...
        try {
            ...
            const reCertificatConformite = /<certificat_conformite>(.*)<\/certificat_conformite>/;
            ...
            for (const url of urls) {
                ...
                if (str.includes('<etat_conformite>KO</etat_conformite>')) {
                    ...
                    if (!bilan || bilan.length !== 2) {
                        throw new Error(`DPAE: no description for non-conformity ${JSON.stringify(this)}\n${str}`);
                    }
                ...
                if (!str.includes('<etat_conformite>OK</etat_conformite>')) {
                    throw new Error(`Should contain conformite ${JSON.stringify(this)}\n${str}`);
                }

                const certif = reCertificatConformite.exec(str);
                if (!certif || certif.length !== 2) {
                    throw new Error(`Cannot find certificat ${JSON.stringify(this)} : ${certif}\n${str}`);
                }

                this.Certificat = certif[1];

                if (this.Certificat.length < 10) {
                    throw new Error(`Incorrect certificat ${JSON.stringify(this)} \n${str}`);
                }
